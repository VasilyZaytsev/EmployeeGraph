...
class EmployeesGraph(inputStream: InputStream) {
   ...
   *
   * @param id HNode id
   * @return Prettified string with resolved nodes
   */
  ...
  /**
   *
   * @return Max level in the graph
   */
  def maxLevel: Int = vMaxLevel

  /**
   * Add HNode to the graph
   * @param str csv row with columns "employee_id","supervisor_id","full_name"
   * @throws IllegalArgumentException in case of invalid input
   * @return
   */
  def addNode(str: String): HNode = str match {
